...
area.mkde <- area.mkde %>%
  full_join(sl.har.final) %>%
                #Bohnsack and Harper
  mutate(mass = ifelse(Species == "Spviride", 0.025*(mSL^2.921), 
                       #Froese ET2014 J. Appl. Ichthyol. (FishBase Bayesian Est.)
                       ifelse(Species == "Scvetula", 0.01445*(mSL^3.03), 
                              #Bohnsack and Harper
                              ifelse(Species == "Spaurofrenatum", 0.00468*(mSL^3.429),
                                     #Bohnsack and Harper
                                     ifelse(Species == "Sctaeniopterus", 0.0135*(mSL^3.0),
                                            #Froese ET2014 (FishBase Bayesian Est.) Sc. iseri
                                            0.01096*(mSL^3.01)))))) %>% 
  rename(SL = mSL,
         Harem = mHarem)
...
nb.c <- glmmTMB(N ~ Site + Species + Int.ID, offset = log(Follow.Time), 
                family=nbinom2, data=tot.pdist.counts)
...
ivBA.l <- pivot_longer(as.data.frame(ivBA), cols = c(2:ncol(ivBA)), names_to = "id2", values_to = "BA95")
...
overlap2 <- subset(overlap, (id1 != id2)) #remove self-overlap (all ~1; removes 170)
...
ovlp <- overlap.n0 %>%
  ...
  mutate(days = abs(difftime(Date1, Date2 , units = c("days")))) %>% #create time between tracks
...
ovlpfin <- overlap2 %>%
  ...
  filter(!grepl("2021-05-", Date1) & !grepl("2021-05-", Date2)) %>% #remove Sp viride before June 2021
  ...
  filter(Phase1 != "IP" & Phase2 != "IP") %>% #remove spatial interactions with IPs 
  filter(Phase1 != "NTP" & Phase2 != "NTP") %>% #remove the non-territorial floaters
  select(-c(2:6, 8:12))
...
sum(neighboring.sum$N) 
#538, good
mean(neighboring.sum$HR.o.perc) 
#75.9%
range(neighboring.sum$HR.o.perc) 
#63.3-83.3% (Sc. taeniopterus - Sc. taeniopterus; Sp. aurofrenatum - Sp. aurofrenatum)
range(1-neighboring.sum$HR.o.perc) 
#16.7-36.7% (Sp. aurofrenatum - Sp. aurofrenatum; Sc. taeniopterus - Sc. taeniopterus)
```
...
BA95.hm <- ggplot(data = TPTP.BA.sum, aes(reorder(Species2, desc(Species2)), Species1, fill = mBA95)) +
...
PHR95overlap2 <- subset(PHR95overlap, (id1 != id2)) %>% #remove self-overlap (all ~1; removes 170)
  left_join(id1.sl.har) %>% #this adds the body size and harem size for each fish
  left_join(id2.sl.har) #this adds the body size and harem size for each fish
...
PHR95ovlp <- PHR95overlap2 %>% 
  separate(id1, c("Site1", "Species1", "Phase1", "Date1", "EndTime1"), sep = "_", remove = F) %>%
  separate(id2, c("Site2", "Species2", "Phase2", "Date2", "EndTime2"), sep = "_", remove = F) %>%
  mutate(days = abs(difftime(Date1, Date2 , units = c("days")))) %>% #create a column fo time bw tracks
  mutate(diff.sl = abs(SL1 - SL2))
```
...
PHR50overlap2 <- subset(PHR50overlap, (id1 != id2)) %>% #remove self-overlap (all ~1; removes 170)
  left_join(id1.sl.har) %>% #this adds the body size and harem size for each fish
  left_join(id2.sl.har) #this adds the body size and harem size for each fish
...
sum(PHR.overlaps.sum$N) 
#1076, good
mean(PHR.overlaps.sum$PHR95.o.perc) 
#77%
range(PHR.overlaps.sum$PHR95.o.perc) 
#63-83% (Sc. taeniopterus - Sc. taeniopterus; Sp. aurofrenatum - Sp. aurofrenatum)

mean(PHR.overlaps.sum$PHR50.o.perc) 
#27%
range(PHR.overlaps.sum$PHR50.o.perc) 
#0-43% (Sp. viride - Sp. viride; Sp. aurofrenatum - Sc. vetula)
```
...
PHR95.hm <- ggplot(data = TPTP.PHR.sum, aes(reorder(Species2, desc(Species2)), Species1, fill = mPHR95)) +
  geom_tile(color = "white") +
...
PHR50.hm <- ggplot(data = TPTP.PHR.sum, aes(reorder(Species2, desc(Species2)), Species1, fill = mPHR50)) +
  geom_tile(color = "white") +
...
TPIPovlp <- ovlp %>%
  ...
  filter(Phase1 != "NTP" & Phase2 != "NTP") %>% #remove all non-territorial terminal phase
...
# Analysis of interaction probabilties as a function of overlap (TOO FEW INTERACTIONS FOR THIS TO BE SUPER HELPFUL)

```{r}
tp.int.counts <- tot.pdist.counts %>%
  ungroup() %>%
  filter(Int.ID %in% c("TP-Conspecific", "TP-Heterospecific")) %>% 
  mutate(Interacted = ifelse(N == 0, "0", "1")) %>%
  rename(N.int = N) %>%
  select(ID, Site, Int.ID, N.int, Interacted) %>%
  mutate_at(vars(Interacted), as.numeric)

# PHR.overlaps has all the overlap data

neighboring.int <- PHR.overlaps %>%
  mutate(Type = ifelse(Species1 == Species2, "Conspecific", "Heterospecific")) %>%
  unite("Int.ID", c("Phase1", "Type"), sep = "-", remove = F) %>%
  select(id1, Species1, Int.ID, SL1, HAR1, BA95, PHR95, PHR50) %>%
  rename(ID = id1,
         Species = Species1,
         SL = SL1,
         HAR = HAR1) %>%
  separate(ID, c("ID", "file"), sep = "[.]", remove = T) %>%
  select(-file) %>%
  group_by(ID, Species, SL, HAR, Int.ID) %>%
  summarise(N = n(),
            mBA95 = mean(BA95),
            cBA95 = sum(BA95),
            mPHR95 = mean(PHR95),
            ...
            maxPHR50 = max(PHR50)) %>%
  ungroup() %>%
  left_join(tp.int.counts) 
  drop_na(N.int, Interacted)

# Overlap (BA)
  
#Binomial
ggplot(data = neighboring.int %>% filter(Int.ID == "TP-Heterospecific"), 
       aes(x = mBA95, y = Interacted)) +
  geom_point() +
  geom_smooth(method = "glm", method.args = list(family = "binomial")) +
  theme_classic()

#Negative Binomial
ggplot(data = neighboring.int %>% filter(Interacted == 1), 
       aes(x = mBA95, y = N.int)) +
  geom_point() +
...
#Binomial
ggplot(data = neighboring.int, aes(x = mPHR95, y = Interacted, color = Int.ID)) +
  geom_point() +
  geom_smooth(method = "glm", method.args = list(family = "binomial")) +
  theme_classic()

#Negative Binomial
ggplot(data = neighboring.int %>% filter(Interacted == 1), aes(x = mPHR95, y = N.int, color = Int.ID)) +
  geom_point() +
  geom_smooth(method = "glm", method.args = list(family = "nbinom2")) +
  theme_classic()
...
#Overlap (PHR50)
#Binomial
ggplot(data = neighboring.int, aes(x = maxPHR50, y = Interacted, color = Int.ID)) +
  geom_point() +
  geom_smooth(method = "glm", method.args = list(family = "binomial")) +
  theme_classic()

#Negative Binomial
ggplot(data = neighboring.int %>% filter(Interacted == 1), aes(x = maxPHR50, y = N.int, color = Int.ID)) +
  geom_point() +
  geom_smooth(method = "glm", method.args = list(family = "nbinom2")) +
  theme_classic()
```
...
ba.int.m1 <- glmmTMB(data = neighboring.int, 
                     Interacted ~ Species*mBA95 + Int.ID*mBA95 + Site, family = binomial())

test.ba.int.m1 <- simulateResiduals(fittedModel = ba.int.m1, n = 250, plot = T) #looks good
testZeroInflation(ba.int.m1) #no significant zero inflation
...
ba.int.m1.anova <- Anova(ba.int.m1, type = 3)
ba.int.m1.anova

# Interaction counts

ba.int.m2 <- glmmTMB(data = neighboring.int, 
                     N.int ~ Species*mBA95 + Species*Int.ID + mBA95*Int.ID + (1|Site:ID), family = poisson())

test.ba.int.m2 <- simulateResiduals(fittedModel = ba.int.m2, n = 250, plot = T) #looks good
testZeroInflation(ba.int.m2) #no significant zero inflation
...
ba.int.m2.anova <- Anova(ba.int.m2, type = 3)
ba.int.m2.anova

# # neighboring.int.con <- neighboring.int %>% 
# #   filter(Int.ID %in% c("TP-Conspecific", "TP-Heterospecific")) 
# 
# neighboring.int.con$fit = predict(ba.int.m1, type = 'response')
# 
# ba.int.p <- ggplot(neighboring.int.con) + 
#   geom_point(aes(x = cBA95, y = Interacted, color = Int.ID)) +
#   stat_smooth(aes(x = cBA95, y = fit, color = Int.ID)) +
#   theme_classic() +
#   theme(axis.title = element_text(size = 12, color = "black"),
#         axis.text = element_text(size = 10, color = "black"),
#         axis.title.x = element_blank(),
...
#         legend.position = "none") +
#   labs(x = "Spatial Overlap", y = "Probability of Interaction")
# ba.int.p

# ggsave("Figures/StaticDynamic/XXXXX.png", plot = ba.int.p,
...
phr95.int.m1 <- glmmTMB(data = neighboring.int, 
                     Interacted ~ Species*mPHR95 + Species*Int.ID + mPHR95*Int.ID+ (1|Site:ID), family = binomial())

test.phr95.int.m1 <- simulateResiduals(fittedModel = phr95.int.m1, n = 250, plot = T) #looks good
testZeroInflation(phr95.int.m1) #no significant zero inflation

#Stats
summary(phr95.int.m1)

#set contrasts for type III unbalanced ANOVA
options(contrasts = c("contr.sum", "contr.poly"))

phr95.int.m1.anova <- Anova(phr95.int.m1, type = 3)
phr95.int.m1.anova
```
...
phr50.int.m1 <- glmmTMB(data = neighboring.int, 
                     Interacted ~ Species*mPHR50 + Species*Int.ID + mPHR50*Int.ID + Site, family = binomial())

test.phr50.int.m1 <- simulateResiduals(fittedModel = phr50.int.m1, n = 250, plot = T) #looks good
testZeroInflation(phr50.int.m1) #no significant zero inflation

#Stats
summary(phr50.int.m1)

#set contrasts for type III unbalanced ANOVA
options(contrasts = c("contr.sum", "contr.poly"))

phr50.int.m1.anova <- Anova(phr50.int.m1, type = 3)
phr50.int.m1.anova
```