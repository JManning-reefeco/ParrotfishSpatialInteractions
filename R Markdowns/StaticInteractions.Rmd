...
write.table(sxp.traj.sum, file = "DataSummary/sxp.traj.sum.txt",
...
write.table(overall.traj.sum, file = "DataSummary/overall.traj.sum.txt",
            sep = ",", quote = FALSE, row.names = F)
```
...
for(i in unique(aqm.sf.met$ID)){
  ...
  ggsave(filename = sprintf('Figures/Stationarity/AQ/%s.png', i), plot = gi)
}
...
for(i in unique(ivm.sf.met$ID)){
  ...
  ggsave(filename = sprintf('Figures/Stationarity/IV/%s.png', i), plot = gi)
}
```
...
write.table(sxp.hr.sum, file = "DataSummary/sxp.hr.sum.txt",
            sep = ",", quote = FALSE, row.names = F)
```
...
ggsave("Figures/Fig1.png", plot = hr.cu.p,
...
ggsave("Figures/FigureS9.png", plot = AQ_SCV_scv_HR,
...
ggsave("Figures/TP_maps_AQ.png", plot = AQ_HRs,
...
ggsave("Figures/TP_maps_IV.png", plot = IV_HRs,
       width = 6.5, height = 7.5, dpi = 300)
```
...
check_collinearity(dur.glm) ##no collinearity between Site, Species, and Int.ID
...
write.table(glmm.int.dur.anova, file = "StatisticalOutputs/Interaction.Dur.GLMM.ANOVA.txt",
...
write.table(nb.c.ANOVA, file = "StatisticalOutputs/Interaction.Count.GLM.ANOVA.txt",
            sep = ",", quote = F, row.names = T)
```
Family: nbinom2  ( log )
...
write.table(glmm.int.dist.edge.anova, 
            file = "StatisticalOutputs/Interaction.Distance.GLMM.ANOVA.txt",
            sep = ",", quote = F, row.names = T)
```
Family: gaussian  ( identity )
...
ggsave("Figures/Fig2.png", plot = interactions.p,
       width = 6, height = 6, dpi = 300)
```
...
ggsave("Figures/FigS2.png", plot = interactions.p2,
...
ggsave("Figures/FigS6.png", plot = aq.scv.pis.p2,
       width = 6, height = 6, dpi = 300)
```
...
ggsave("Figures/FigS7.png", plot = buffers.p,
       width = 6, height = 6, dpi = 300)
```
...
ggsave("Figures/BA95_model.png", plot = overlap.emm.p,
...
ggsave("Figures/BA95_model_pwpp.png", plot = BA95.emm.pwpp,
...
ggsave("Figures/PHR95_model.png", plot = PHR95.emm.p,
       width = 6.5, height = 4, dpi = 300)
```
Family: beta  ( logit )
...
ggsave("Figures/PHR95_model_pwpp.png", plot = PHR95.emm.pwpp,
       width = 6.5, height = 8, dpi = 300)
```
...
ggsave("Figures/PHR50_model.png", plot = PHR50.emm.p,
       width = 6.5, height = 4, dpi = 300)
```
Family: beta  ( logit )
...
ggsave("Figures/PHR50_model_pwpp.png", plot = PHR50.emm.pwpp,
       width = 6.5, height = 8, dpi = 300)
```
...
ggsave("Figures/Fig3.png", plot = overlaps.p,
...
write.table(TPIPovlp.sum, file = "DataSummary/TPIPoverlap.txt", 
            sep = ",", quote = FALSE, row.names = F)
...
ggsave("Figures/FigS8.png", plot = TPIP.SO.p,
...
# ggsave("Figures/FigSX2.png", plot = TPIP.PHR95.p,
#        width = 4, height = 4, dpi = 300)
...
# ggsave("Figures/FigSX3.png", plot = TPIP.PHR50.p,
#        width = 4, height = 4, dpi = 300)
```
