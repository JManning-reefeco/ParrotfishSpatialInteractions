...
# Analysis of interaction probabilties as a function of overlap
...
neighboring.int.con <- neighboring.int %>%
  filter(Int.ID == "TP-Conspecific" & N.int > 0)

cor.test(neighboring.int.con$mBA95, neighboring.int.con$N.int, method=c("pearson"))

ggplot(data = neighboring.int.con, aes(x = mBA95, y = N.int)) +
  geom_point()



ba.int.m1 <- glmmTMB(data = neighboring.int %>% filter(Int.ID == "TP-Conspecific" & mBA95 > 0), Interacted ~ mBA95 + 
                      (1|Site:ID), family = binomial())

test.ba.int.m1 <- simulateResiduals(fittedModel = ba.int.m1, n = 250, plot = T) #looks good
testDispersion(test.ba.int.m1) #meets dispersion assumptions
testOutliers(test.ba.int.m1) #outlier test is not significant
...
```{r}
PHR95.int.m <- glmmTMB(data = neighboring.int %>% filter(Int.ID == "TP-Conspecific"), Interacted ~ mPHR50 + 
                         Species + (1|Site:ID), offset = log(N), family = binomial())

test.PHR95.int.m <- simulateResiduals(fittedModel = PHR95.int.m, n = 250, plot = T) #looks good
testDispersion(test.PHR95.int.m) #meets dispersion assumptions
testOutliers(test.PHR95.int.m) #outlier test is not significant
testZeroInflation(PHR95.int.m) #no significant zero inflation
...
PHR95.int.m.anova <- Anova(PHR95.int.m, type = 3)
PHR95.int.m.anova

neighboring.int$fit2 <- predict(PHR95.int.m, type = 'response')

PHR95.int.p <- ggplot(neighboring.int) + 
  geom_point(aes(x = mPHR95, y = N.int, color = Int.ID)) +
  geom_smooth(aes(x = mPHR95, y = fit2, color = Int.ID)) +
  theme_classic() +
  theme(axis.title = element_text(size = 12, color = "black"),
        axis.text = element_text(size = 10, color = "black"),
        axis.title.x = element_blank(),
        axis.text.x = element_text(color = "black"),
        legend.position = "none") +
  labs(x = "Spatial Overlap", y = "Counts")
PHR95.int.p

# ggsave("Figures/StaticDynamic/XXXXX.png", plot = PHR95.int.p,
#        width = 3.25, height = 4, dpi = 300)
```
...
```{r}
neighboring.int.red <- neighboring.int %>% filter(!(Species == "Spviride" & Int.ID == "TP-Conspecific"))

PHR50.int.m <- glmmTMB(data = neighboring.int.red, 
                       N.int ~ Species + Int.ID + mPHR50 + 
                      (1|Site:ID), family = poisson())

PHR50.int.m <- glmmTMB(data = neighboring.int.red, N.int ~ mPHR50:Int.ID + 
                         Species + Int.ID + mPHR50 + 
                      (1|Site:ID), family = poisson())

test.PHR50.int.m <- simulateResiduals(fittedModel = PHR50.int.m, n = 250, plot = T) #looks good
testDispersion(test.PHR50.int.m) #meets dispersion assumptions
testOutliers(test.PHR50.int.m) #outlier test is not significant
testZeroInflation(PHR50.int.m) #no significant zero inflation
...
PHR50.int.m.anova <- Anova(PHR50.int.m, type = 3)
PHR50.int.m.anova

neighboring.int.red$fit3 <- predict(PHR50.int.m, type = 'response')

PHR50.int.p <- ggplot(neighboring.int.red) + 
  geom_point(aes(x = mPHR50, y = N.int, color = Int.ID)) +
  stat_smooth(aes(x = mPHR50, y = fit3, color = Int.ID)) +
  theme_classic() +
  theme(axis.title = element_text(size = 12, color = "black"),
        axis.text = element_text(size = 10, color = "black"),
        axis.title.x = element_blank(),
